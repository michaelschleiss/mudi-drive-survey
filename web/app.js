...
function uploadCarrierText(st){
  const live=!!st.busy, evidence=live?st.active?.uplink:st.latest.probe?.uplink;
  if(!evidence||live&&age(evidence)>3)return live?'Reading uplink carriers…':'No carrier readings for this test';
  const carriers=evidence.carriers||[];
  if(!carriers.length)return 'Uplink carrier readings unavailable';
  return `${live?'Reported UL layers':'UL layers seen during test'}: `+carriers.map(c=>`${c.band||`${c.rat.toUpperCase()} channel ${c.channel}`} ${live?c.ul_layers:c.max_ul_layers}`).join(' · ');
}
function renderUploadControls(){
  ...
  for(const id of ['toggle','drive-pause']){
    ...
    button.textContent=uploadControlPending?(uploadControlTarget?'Starting test…':'Stopping test…'):(running?'Stop test':state?.busy?'Finishing current test…':'Test upload here ↑');
  }
}
...
function renderTestHistory(){
  ...
  for(const e of recordedTests.slice(-100).reverse()){
    ...
    if(e.estimate_count)method.textContent+=` · request median estimate ${fmt(e.estimate_median_mbps,1)} / P90 ${fmt(e.estimate_p90_mbps,1)} Mbps`;
...
function render(st){
  ...
  $('upload-heading').textContent=st.busy?'UPLOAD TEST IN PROGRESS':'LATEST COMPLETED UPLOAD';
  $('probe-status').textContent=st.busy?'20-SECOND TEST':st.running?'PREPARING':latestValid?'COMPLETE':'READY';
  $('upload').textContent=st.busy?fmt(active?.mbps):latestValid?fmt(latestValid.mbps):'—';
  $('upload-detail').textContent=st.busy?`${active?.mbps!=null?'Average so far':'Starting measurement…'} · ${fmt(Date.now()/1000-(active?.start||Date.now()/1000))} s elapsed`:st.running?'Preparing next test…':latestValid?`${fmt(age(latestValid))} s ago · completed upload test`:'Ready for your first upload test';
  $('upload-uplink').textContent=uploadCarrierText(st);
...
async function tick(){
  ...
  }catch(e){failed=true;$('connection').textContent='OFFLINE';$('notice').textContent='Connection lost. Displayed readings may be stale. Reconnecting…';$('notice').classList.add('warn');$('upload').textContent='—';$('traffic').textContent='—';$('accuracy').textContent='—';window.driveOffline?.();}
  // Schedule from request start; never overlap requests or queue stale renders.
  setTimeout(tick,Math.max(25,(failed?1000:250)-(performance.now()-started)));
}
tick();
