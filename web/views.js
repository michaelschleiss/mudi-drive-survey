...
(() => {
 ...
 function renderVerification(st){
  const a=st.active,p=st.latest.probe,requests=st.busy?(a?.requests||[]):(p?.request_measurements||[]);
  ...
  if(st.busy||average!=null){const metric=document.createElement('div');metric.className='verification-speed';const value=document.createElement('b'),unit=document.createElement('span');value.textContent=fmt(average,1);unit.textContent='Mbps ↑';metric.append(value,unit);verification.append(metric);
   const label=document.createElement('p');label.textContent=st.busy?'Average so far':'Measured upload speed';verification.append(label);}
  const carriers=document.createElement('p');carriers.className='upload-carrier-evidence';carriers.textContent=uploadCarrierText(st);verification.append(carriers);
  const caveat=document.createElement('small');caveat.textContent='Reported layers · per-band traffic unconfirmed';verification.append(caveat);
  if(requests.length){const details=document.createElement('details'),summary=document.createElement('summary');summary.textContent='Details';details.append(summary);
   for(const [i,r] of requests.entries()){const row=document.createElement('p');row.textContent=`${i+1}. ${fmt(r.bytes/1e6,1)} MB · ${fmt(r.request_seconds,3)} s request · ${fmt(r.estimate_mbps,1)} Mbps estimate · ${r.accepted?'accepted':'failed'} · connection ${r.connection_reused?'reused':'new'} · server timing ${fmt(r.server_reported_ms,2)} ms`;details.append(row);}verification.append(details);}
 }
